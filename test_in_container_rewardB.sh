...
# Evaluates the rewardB checkpoint (R_total = R_answer - lambda * beta * redundancy)
# on the CDs_and_Vinyl test set using the same 4-step pipeline as test_in_container_ablationA.sh.
#
# Usage:
#   bash test_in_container_rewardB.sh
#
# Overrideable env vars (set before running):
#   CHECKPOINT_STEP   — default "latest"; can be "global_step_500", "500", etc.
#   FORCE_MERGE       — set to 1 to re-merge even if merged model already exists
...
DEFAULT_EVAL_DIR="$PROJECT_DIR/outputs/reward/$EXPERIMENT_NAME/$CHECKPOINT_SUBDIR"
...
singularity exec --nv --writable-tmpfs \
    ...
    bash -c '
        ...
        # Strip cuda/compat from LD_LIBRARY_PATH so host driver (CUDA 13.1) is used
        export LD_LIBRARY_PATH=$(echo "${LD_LIBRARY_PATH:-}" | tr ":" "\n" | grep -v "cuda/compat" | paste -sd ":" -)
        echo "[DEBUG] LD_LIBRARY_PATH after stripping: $LD_LIBRARY_PATH"
        echo "[DEBUG] GPU check:" && python3 -c "import torch; print(f\"CUDA available: {torch.cuda.is_available()}, devices: {torch.cuda.device_count()}\")" || echo "[WARN] GPU check failed"
...
for idx in range(len(responses)):
    ...
    records.append({"input": question, "output": target, "predict": preds_list})
