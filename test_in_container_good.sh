...
export DATA_DIR=${DATA_DIR:-'./data/goodreads_data'}
...
export EXPERIMENT_NAME=${EXPERIMENT_NAME:-nq-search-r1-grpo-qwen3-1.7b-mt4-3072-goodreads}
...
EVAL_CATEGORY=${EVAL_CATEGORY:-'Goodreads'}
# Optional explicit dataset resource dir (contains id2name.json, name2id.json, embeddings.pt, ...).
# If unset, it is derived from EVAL_CATEGORY under $PROJECT_DIR/data/goodreads_data/<category>.
...
PY
        ...
        #   EVAL_DATASET_DIR=$PROJECT_DIR/data/goodreads_data/CDs_and_Vinyl   (optional)
        #
        # If EVAL_CATEGORY is a directory path, we treat it as dataset_dir and use its basename as category.
        # If EVAL_CATEGORY is a plain category name, derive dataset_dir under $PROJECT_DIR/data/goodreads_data/.
        if [[ -n "${EVAL_DATASET_DIR:-}" ]]; then
            ...
            if [[ -d "$_candidate_dir" ]]; then
                ...
                _eval_dataset_dir="$PROJECT_DIR/data/goodreads_data/$_eval_category_name"
            fi
        else
            _eval_category_name="$EVAL_CATEGORY"
            _eval_dataset_dir="$PROJECT_DIR/data/goodreads_data/$EVAL_CATEGORY"
        fi
