...
#SBATCH -n 2
#SBATCH -t 24:00:00
#SBATCH -o output_dual_gpu_baseline.log

# OUTCOME-ONLY BASELINE (no reasoning reward) — reference for rthink / rewardB.
# Reward = R_answer only (reward_SPRec); reasoning reward disabled.
#
#   node[0] → retriever (retrieval_launch.sh)
#   node[1] → training  (run_in_container_baseline.sh, USE_RTHINK=0 USE_REWARD_B=0)
#
...
srun --nodelist="${training_host}" --nodes=1 --ntasks=1 --exclusive --chdir="${PROJECT_DIR}" \
  bash run_in_container_baseline.sh &
training_pid=$!
