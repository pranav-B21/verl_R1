...
#SBATCH -n 2
#SBATCH -t 24:00:00
#SBATCH -o output_dual_gpu_ablationB.log
...
start_retriever() {
  ...
  srun --nodelist="${retriever_host}" --nodes=1 --ntasks=1 --exclusive bash -lc \
    "source ~/.bashrc && conda activate retriever && cd ${PROJECT_DIR} && bash retrieval_launch_ablationB.sh" &
  retrieval_pid=$!
}
...
srun --nodelist="${training_host}" --nodes=1 --ntasks=1 --exclusive --chdir="${PROJECT_DIR}" bash run_in_container_ablationB.sh "${training_args[@]}" &
training_pid=$!
