...
#SBATCH -n 2
#SBATCH -t 12:00:00
#SBATCH -J test_v7b_n8
#SBATCH -o output_test_v7b_n8_%j.log

# Held-out eval for the M3 CORRECTED treatment arm (v7b, length-penalty fixes):
#   nq-search-r1-grpo-qwen3-1.7b-sbatch-gpu-rthink-v7b-n8 @ global_step_200
#
# Submit from a LOGIN node (sbatch is unavailable on compute nodes):
#   sbatch sbatch_run_test_v7b_n8.sh
#   EVAL_REPEATS=1 sbatch sbatch_run_test_v7b_n8.sh    # behavior-only, ~1 decode
#   CHECKPOINT_STEP=150 sbatch sbatch_run_test_v7b_n8.sh
#
...
#
# Comparison arms at the SAME step 200:
#   sbatch_run_test_v7_n8.sh       -- buggy length penalty, retrieval ~5%
#   sbatch_run_test_baseline_n8.sh -- outcome-only, retrieval ~99%
# Both already have step-200 decodes on disk, so neither needs re-running unless
# you want same-day parity.
#
...
export TEST_SCRIPT=test_in_container_v7b_n8.sh
